...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
    ...
    int bytes2Write = size, totalWrite = 0, error;
    ...
    node->modificationTime = time(NULL);
    error = updateSuperBlock(&myFileSystem);
...
static int my_unlink(const char *path) {
	...
	entry = findFileByName(&myFileSystem, path_mod);
	
	if (entry < 0) 
		return -1;

	//idxNodoI = nodo-i del fichero
	...
	free(myFileSystem.nodes[idxNodoI]); //free_null?
	free(path_mod);
...
static int my_read(const char *path,  char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);
	 ...
	 /* Cualquier intento de leer más allá de fin de fichero -> devolver 0 bytes*/
