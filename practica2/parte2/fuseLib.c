...
static int my_write(const char *path, const char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    node->modificationTime = time(NULL);
    int error = updateSuperBlock(&myFileSystem);
...
static int my_unlink(const char *path) {
	...
	char *path_mod = malloc(sizeof(char)*64);
	strcpy(path_mod, path);

	// Remove beginning slash
	size_t len = strlen(path_mod);
	if((len > 0) && (path_mod[0] == '/')) {
		memmove(path_mod, path_mod+1, len);
	}
	
	// Buscar path en el directorio del SF
	int entry = findFileByName(&myFileSystem, path_mod);
	
	if (entry < 0) 
		return -1;

	idxNodoI = myFileSystem.directory.files[entry].nodeIdx;
	...
	myFileSystem.directory.files[entry].freeFile = 1;
	myFileSystem.directory.numFiles--;
	...
	if (updateNode(&myFileSystem, idxNodoI, myFileSystem.nodes[idxNodoI]) < 0)
		return -1;
...
static int my_read(const char *path,  char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	char buffer[BLOCK_SIZE_BYTES];
	int bytesParaLeer = size, totalRead = 0;
	NodeStruct *nodoI = myFileSystem.nodes[fi->fh];

	fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);

	if (offset >= myFileSystem.nodes[fi->fh]->fileSize)
	    return 0;

	if (size + offset > myFileSystem.nodes[fi->fh]->fileSize)
		bytesParaLeer = myFileSystem.nodes[fi->fh]->fileSize - offset;
	...
	while (bytesParaLeer) {
		...
		bloqueActual = nodoI->blocks[offset / BLOCK_SIZE_BYTES];
		...
		if ((lseek(myFileSystem.fdVirtualDisk, bloqueActual * BLOCK_SIZE_BYTES, SEEK_SET) == (off_t) - 1) ||
				...
				perror("Falló read en my_read");
				return -EIO;
		}

		for(i = offBloque; (i < BLOCK_SIZE_BYTES) && (bytesParaLeer > 0); i++){
...
int my_rename(const char * oldfile, const char *newfile){
	...
	// Si el nombre es mayor que el tamaño máximo
	if((strlen(newfile) + 1) > MAX_LEN_FILE_NAME){
		return -ENOSPC;
	}

	// Si el fichero a renombrar no existe, devuelve error
	if ((idxNodo = findFileByName(&myFileSystem, (char*)oldfile+1)) == -1) {
		return -ENOENT;
	}

	// Si ya hay un fichero con nombre newfile, lo borra
	if (findFileByName(&myFileSystem, (char*)newfile+1) != -1) {
		my_unlink(newfile);
	}
	
	// Renombra el fichero
	strcpy(myFileSystem.directory.files[idxNodo].fileName, newfile+1);
	updateDirectory(&myFileSystem);
	sync();
...
int my_symlink(const char *existingpath, const char *newpath)
{
	...
	if (strlen(newpath+1) > myFileSystem.superBlock.maxLenFileName) {
		return -ENAMETOOLONG;
	}
	...
	if( findFileByName(&myFileSystem, (char*)newpath+1) != -1){
		return -EEXIST;
	}
	...
	int idxNode;
	if ((idxNode = findFreeNode(&myFileSystem)) == -1) {
		return -ENOSPC;
	}

	//Actualizamos el directorio raiz
	myFileSystem.directory.files[idxNode].freeFile = false;
	myFileSystem.directory.numFiles++;
	strcpy(myFileSystem.directory.files[idxNode].fileName, newpath+1);
	myFileSystem.directory.files[idxNode].nodeIdx = idxNode;
	myFileSystem.numFreeNodes--;


	//Rellenamos los campos del inodo nuevo
	if (myFileSystem.nodes[idxNode] == NULL) {
		myFileSystem.nodes[idxNode] = malloc(sizeof(NodeStruct));
	}
	myFileSystem.nodes[idxNode]->fileSize = strlen(existingpath);
	myFileSystem.nodes[idxNode]->numBlocks = 0;
	myFileSystem.nodes[idxNode]->fileType = 1;
	strcpy(myFileSystem.nodes[idxNode]->slinkDestination, existingpath);
	myFileSystem.nodes[idxNode]->modificationTime = time(NULL);
	myFileSystem.nodes[idxNode]->freeNode = false;


	reserveBlocksForNodes(&myFileSystem, myFileSystem.nodes[idxNode]->blocks, 0);
...
int my_readlink(const char *path, char *buf, size_t tambuf){
	...
	int dir_idx, idxNodoI;
	FileStruct *entryDirectory = NULL;
	NodeStruct *nodoI = NULL;
	...
	entryDirectory = &myFileSystem.directory.files[dir_idx];
	idxNodoI = entryDirectory->nodeIdx;
	nodoI = myFileSystem.nodes[idxNodoI];

	if (nodoI->fileType != 1)
		return -EINVAL;

	if (tambuf<strlen(nodoI->slinkDestination))
		strcpy(buf,nodoI->slinkDestination);
	else {
		strncpy(buf,nodoI->slinkDestination,tambuf-1);
		buf[tambuf-1]='\0';
	}
    return 0;
}
